...
API呼び出しは行わず、プロンプトを prompt.md に保存する。
Claude Code がそのプロンプトを読んで all_posts.md を生成する。
"""
import re
...
def save_prompt(prompt: str, week_start: datetime, posts_dir) -> Path:
    """プロンプトを prompt.md に保存して返す"""
    week_dir = posts_dir / f"{week_start.strftime('%Y-%m-%d')}_week"
    week_dir.mkdir(parents=True, exist_ok=True)

    prompt_file = week_dir / "prompt.md"
    prompt_file.write_text(prompt, encoding="utf-8")
    return week_dir, prompt_file


def save_posts(content: str, week_dir: Path):
    """生成済み投稿文を all_posts.md に保存する"""
...
def run(ctx=None):
    ...
    week_dir, prompt_file = save_prompt(prompt, next_monday, ctx.posts_dir)
    print(f"✅ プロンプト保存完了: {prompt_file}")
    print(f"\n【Claude Codeセッション対応】")
    print(f"プロンプトを読んで all_posts.md を生成してください:")
    print(f"  prompt: {prompt_file}")
    print(f"  output: {week_dir}/all_posts.md")
